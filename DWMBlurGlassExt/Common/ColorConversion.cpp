...
MDWMBlurGlassExt::Rgb::Rgb(double r, double g, double b) : r{ r }, g{ g }, b{ b }
{
}

MDWMBlurGlassExt::Hsv::Hsv(double h, double s, double v) : h{ h }, s{ s }, v{ v }
{
}

std::optional<unsigned long> MDWMBlurGlassExt::TryParseInt(const std::wstring_view& s)
{
    return TryParseInt(s, 10 /* base */);
}

std::optional<unsigned long> MDWMBlurGlassExt::TryParseInt(const std::wstring_view& str, int base)
{
...
MDWMBlurGlassExt::Hsv MDWMBlurGlassExt::RgbToHsv(const Rgb& rgb)
{
    double hue = 0;
    double saturation = 0;
    double value = 0;
...
MDWMBlurGlassExt::Rgb MDWMBlurGlassExt::HsvToRgb(const MDWMBlurGlassExt::Hsv& hsv)
{
    double hue = hsv.h;
    ...
    if (chroma == 0)
    {
        return MDWMBlurGlassExt::Rgb(min, min, min);
    }
    ...
    return MDWMBlurGlassExt::Rgb(r, g, b);
}

MDWMBlurGlassExt::Rgb MDWMBlurGlassExt::HexToRgb(const std::wstring_view& input)
{
    const auto [rgb, a] = HexToRgba(input);
    return rgb;
}

winrt::hstring MDWMBlurGlassExt::RgbToHex(const Rgb& rgb)
{
...
std::tuple<MDWMBlurGlassExt::Rgb, double> MDWMBlurGlassExt::HexToRgba(const std::wstring_view& input)
{
...
winrt::hstring MDWMBlurGlassExt::RgbaToHex(const MDWMBlurGlassExt::Rgb& rgb, double alpha)
{
...
winrt::Windows::UI::Color MDWMBlurGlassExt::ColorFromRgba(const MDWMBlurGlassExt::Rgb& rgb, double alpha)
{
...
MDWMBlurGlassExt::Rgb MDWMBlurGlassExt::RgbFromColor(const winrt::Windows::UI::Color& color)
{
